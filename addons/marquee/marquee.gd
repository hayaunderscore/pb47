...
var fon: Font = preload("res://gfx/font/monogram-extended.ttf")
...
func _pseudo_rebuild():
	var min_size : Vector2 = Vector2(0, 0)
	for line in lines:
		if _labels.size() > 0 and _labels[-1].text == line: continue
		var label = Label.new()
		label.text = line
		label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
		if label_variation.length() > 0:
...
func _relayout():
	...
	for lidx in range(_labels.size()):
		...
		if lidx == 0:
			...
			_tween.tween_property(current_label, "position", center - next_label.get_rect().size / 2.0, duration).from(current_label.position)
		else:
			current_label.position = Vector2(get_rect().size.x, center.y - next_label.get_rect().size.y / 2.0)
		...
		if delete_after_use:
			...
			_tween.tween_callback(current_label.queue_free)
